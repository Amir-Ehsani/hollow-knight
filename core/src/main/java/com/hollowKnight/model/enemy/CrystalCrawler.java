...
public class CrystalCrawler implements Damageable {

    public enum State {
        WALKING, TURNING, DYING, DEAD
    }

    public static final float DEFAULT_HITBOX_WIDTH = 78f;
    public static final float DEFAULT_HITBOX_HEIGHT = 50f;
    public static final int DEFAULT_HEALTH = 3;
    public static final float DEFAULT_SPEED = 70f;
    ...
    private static final float DEATH_DURATION = 0.58f;
    ...
    public void update(float delta, Array<Rectangle> platforms, Array<Polygon> polygonPlatforms) {
        ...
        if (hurtKnockbackTimer > 0f) {
            ...
            moveAxis(moveX, 0f, platforms, polygonPlatforms);
            applyGravityAndVerticalCollision(delta, platforms, polygonPlatforms);
        ...
        float moveX = velocity.x * delta;
        boolean hitWall = moveAxis(moveX, 0f, platforms, polygonPlatforms);
    ...
    private void applyGravityAndVerticalCollision(float delta, Array<Rectangle> platforms, Array<Polygon> polygonPlatforms) {
        ...
        boolean hitVertical = moveAxis(0f, moveY, platforms, polygonPlatforms);
    ...
    private boolean isAtCliffEdge(Array<Rectangle> platforms, Array<Polygon> polygonPlatforms) {
        ...
        if (direction > 0) {
            probeX = bounds.x + bounds.width + CLIFF_CHECK_DISTANCE_X;
        } else {
            probeX = bounds.x - CLIFF_CHECK_DISTANCE_X;
        }
    ...
    private boolean isTouchingGround(Array<Rectangle> platforms, Array<Polygon> polygonPlatforms) {
        probeBounds.set(bounds.x + 4f, bounds.y - GROUND_CHECK_DISTANCE, bounds.width - 8f, GROUND_CHECK_DISTANCE + 1f);
        return overlapsAny(probeBounds, platforms, polygonPlatforms);
    }
    ...
    public void takeDamage(int damage, float knockbackX, float knockbackY) {
        ...
        if (currentHealth <= 0) {
            ...
            velocity.y = Math.max(velocity.y, knockbackY);
            grounded = false;
        }
    }
